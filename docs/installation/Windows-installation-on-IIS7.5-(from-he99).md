(Cloned from he99's guide: https://github.com/he99/ampache/wiki/Windows-installation-on-IIS7.5)
...
- Streaming Radio stations only work with mp3 streams (m3u playlists don't work, this is an already known issue, see https://github.com/ampache/ampache/issues/1313)
...
- Git, composer etc. installed (only required if you want to clone and build Ampache as described in the manual at https://github.com/ampache/ampache/wiki/Installation)
...
As I had already installed another PHP version on the server, I've also installed PHPManager V2.4 from https://github.com/phpmanager/phpmanager. At the first run it showed a few suggestions - I followed them all.
...
To enable the charts, an additional install step is needed (see https://github.com/ampache/ampache/wiki/chart-faq):
...
See https://stackoverflow.com/questions/14934006/iis-iusrs-and-iusr-permissions-in-iis8
...
I've also enabled Failed Request tracking (see https://docs.microsoft.com/en-us/iis/extensions/url-rewrite-module/using-failed-request-tracing-to-trace-rewrite-rules) to actually see what IIS comes up with.

The only rewrite rule I've implemented is for the `ampache/rest` folder - this is required to make Subsonic clients work. The directory has a sample .htaccess.dist file, which I've converted to an IIS rule as follows (I've got some hints from https://groups.google.com/forum/#!topicsearchin/ampache/subject$3Aiis/ampache/TIkvVFG3GRU):
